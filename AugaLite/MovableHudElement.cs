...
namespace AugaLite
{
    public class MovableHudElement : MonoBehaviour
    {
        ...
        {
            Anchor = AugaLite.instance.Config.Bind(nameOverride, $"{nameOverride}Anchor", defaultAnchor, $"Anchor for {nameOverride}");
            Position = AugaLite.instance.Config.Bind(nameOverride, $"{nameOverride}Position", new Vector2(defaultPositionX, defaultPositionY), $"Position for {nameOverride}");
            Scale = AugaLite.instance.Config.Bind(nameOverride, $"{nameOverride}Scale", 1.0f, $"Uniform scale for {nameOverride}");
        }
