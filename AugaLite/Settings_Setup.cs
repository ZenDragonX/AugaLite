...
namespace AugaLite
{
    ...
    public static class Settings_Setup
    {
        ...
        public static void SetupResolutionOptions(Settings __instance)
        {
            ...
            optionList.AddRange(__instance.m_resolutions.Select(x => $"{x.width}x{x.height} {x.refreshRate}hz").ToList());
        ...
        public static bool UpdateBindings_Prefix(Settings __instance)
        {
            foreach (var key in __instance.m_keys)
            {
                ...
                if (!bindingDisplay)
                {
                    AugaLite.LogWarning($"Could not set binding for {key.m_keyTransform.name}");
                    continue;
                }
